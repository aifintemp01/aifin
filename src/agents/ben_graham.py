...
def ben_graham_agent(
    state: AgentState,
    agent_id: str = "ben_graham_agent",
    layer: int = 1,
    ...
    Layer-aware: reads upstream context when in Layer 2+, writes to layer_context always,
    writes to analyst_signals only when is_last_hidden=True.
    """
    data = state["data"]
    ...
    for ticker in tickers:
        ...
        layer_context_updates[f"{agent_id}:{ticker}"] = {
            ...
            "confidence": graham_output.confidence,
            "key_findings": [
                f"Graham score: {total_score}/{max_possible_score}",
                f"Earnings: {earnings_analysis.get('details', 'N/A')[:80]}",
                f"Strength: {strength_analysis.get('details', 'N/A')[:80]}",
            ],
            "data_summary": graham_output.reasoning[:300],
        }
...
def analyze_financial_strength(financial_line_items: list) -> dict:
    ...
    div_periods = [_safe_get(item, "dividends_and_other_cash_distributions") for item in financial_line_items
...
def generate_graham_output(
    ...
    if upstream_context:
        ...
        upstream_section += "\nIncorporate this context into your Graham-style assessment.\n"
    ...
    template = ChatPromptTemplate.from_messages([
        (
            ...
            Be thorough and specific: reference exact metrics, Graham's thresholds, and use his conservative voice.
