...
class ApiKeysService {
  ...
  async getApiKey(provider: string): Promise<ApiKey> {
    const response = await apiFetch(`${this.baseUrl}/${encodeURIComponent(provider)}`);
    if (!response.ok) {
  ...
  async deactivateApiKey(provider: string): Promise<ApiKeySummary> {
    const response = await apiFetch(`${this.baseUrl}/${encodeURIComponent(provider)}/deactivate`, {
      method: 'PATCH',
    });
  ...
  async bulkUpdateApiKeys(request: ApiKeyBulkUpdateRequest): Promise<ApiKey[]> {
    const response = await apiFetch(`${this.baseUrl}/bulk`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async updateLastUsed(provider: string): Promise<void> {
    const response = await apiFetch(`${this.baseUrl}/${encodeURIComponent(provider)}/last-used`, {
      method: 'PATCH',
    });
